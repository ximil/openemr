...
class C_Document extends Controller {
	...
	function validate_action_process($patient_id="", $document_id) {
		...
		if($d->couch_docid && $d->couch_revid){
			$couch = new CouchDB();
			$resp = $couch->retrieve_doc(array($GLOBALS['couchdb_dbase'],$d->couch_docid));
			$current_hash = $resp->_rev;
			$messages = xl('Current CouchDB Rev ID').": ".$current_hash."<br>";
			$messages .= xl('Stored CouchDB Rev ID').": ".$d->couch_revid."<br>";
			if ( $current_hash != $d->couch_revid ) {
				$messages .= xl('CouchDB Revision ID does not match. Data integrity has been compromised.');
			} else
			{
