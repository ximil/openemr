...
function ModifyPayments()
 {//Used while modifying the allocation
	...
	if(CompletlyBlankAbove())//The distribution rows already in the database are checked.
	 {
	  alert("<?php htmlspecialchars( xl('None of the Top Distribution Row Can be Completly Blank.','e'), ENT_QUOTES);echo htmlspecialchars('\n');htmlspecialchars( xl('Use Delete Option to Remove.','e'), ENT_QUOTES) ?>")
	  return false;
	 }
	 ...
	 {
	  alert("<?php htmlspecialchars( xl('Fill any of the Below Row.','e'), ENT_QUOTES) ?>")
	  return false;
	 }
	PostValue=CheckUnappliedAmount();//Decides TdUnappliedAmount >0, or <0 or =0
	if(PostValue==1)
	 {
	  alert("<?php htmlspecialchars( xl('Cannot Modify Payments.Undistributed is Negative.','e'), ENT_QUOTES) ?>")
	  return false;
	 }
	if(confirm("<?php htmlspecialchars( xl('Would you like to Modify Payments?','e'), ENT_QUOTES) ?>"))
	 {
...
function FinishPayments()
 {
	...
	if(PostValue==2)
	 {
		if(confirm("<?php htmlspecialchars( xl('Would you like to Modify and Finish Payments?','e'), ENT_QUOTES) ?>"))
		 {
			UnappliedAmount=document.getElementById('TdUnappliedAmount').innerHTML*1;
			if(confirm("<?php htmlspecialchars( xl('UNDISTRIBUTED is ','e'), ENT_QUOTES) ?>" + UnappliedAmount + '.' + "<?php echo htmlspecialchars('\n');htmlspecialchars( xl('Would you like the balance amount to apply to Global Account?','e'), ENT_QUOTES) ?>"))
			 {
...
function OnloadAction()
 {//Displays message while loading after some action.
  ...
  if(after_value=='Modify' || after_value=='Finish')
   {
    alert("<?php htmlspecialchars( xl('Successfully Modified','e'), ENT_QUOTES) ?>")
	return true;
   }
  ...
  if(after_value=='distribute')
  ...
  else if(after_value=='new_payment')
   {
	...
	if(confirm("<?php htmlspecialchars( xl('Successfully Saved.Would you like to Distribute?','e'), ENT_QUOTES) ?>"))
	 {
		if(document.getElementById('TablePatientPortion'))
		 {
...
function DeletePaymentDistribution(DeleteId)
 {//Confirms deletion of payment distribution.
	if(confirm("<?php htmlspecialchars( xl('Would you like to Delete Payment Distribution?','e'), ENT_QUOTES) ?>"))
	 {
...
<?php
  ...
  <tr>
    <td colspan="3" align="left">
		<ul class="tabNav">
		 <li class='current'><a href='new_payment.php'><?php htmlspecialchars( xl('New Payment','e'), ENT_QUOTES) ?></a></li>
		 <li><a href='search_payments.php'><?php htmlspecialchars( xl('Search Payment','e'), ENT_QUOTES) ?></a></li>
					...
					do 
					 {
						...
						$ResultSearch = sqlStatement("SELECT billing.id,last_level_closed,billing.encounter,form_encounter.`date`,billing.code,billing.modifier,fee
						 ...
						 form_encounter.pid ='$PId' and billing.encounter in (SELECT distinct encounter from ar_activity where  session_id ='$payment_id' )
						...
						<table width="1004"  border="0" cellpadding="0" cellspacing="0" align="center" id="TableDistributePortion">
						  <tr class="text" bgcolor="#dddddd">
						    ...
						    <td width="144" class="left top" ><?php htmlspecialchars( xl('Patient Name','e'), ENT_QUOTES) ?></td>
							<td width="25" class="left top" ><?php htmlspecialchars( xl('For','e'), ENT_QUOTES) ?></td>
							<td width="70" class="left top" ><?php htmlspecialchars( xl('Srv Date','e'), ENT_QUOTES) ?></td>
							<td width="50" class="left top" ><?php htmlspecialchars( xl('Encnter','e'), ENT_QUOTES) ?></td>
							<td width="65" class="left top" ><?php htmlspecialchars( xl('CPT Code','e'), ENT_QUOTES) ?></td>
							<td width="50" class="left top" ><?php htmlspecialchars( xl('Charge','e'), ENT_QUOTES) ?></td>
							<td width="40" class="left top" ><?php htmlspecialchars( xl('Copay','e'), ENT_QUOTES) ?></td>
							<td width="40" class="left top" ><?php htmlspecialchars( xl('Remdr','e'), ENT_QUOTES) ?></td>
							<td width="60" class="left top" ><?php htmlspecialchars( xl('Allowed','e'), ENT_QUOTES) ?></td>
							<td width="60" class="left top" ><?php htmlspecialchars( xl('Payment','e'), ENT_QUOTES) ?></td>
							<td width="70" class="left top" ><?php htmlspecialchars( xl('Adj Amount','e'), ENT_QUOTES) ?></td>
							<td width="60" class="left top" ><?php htmlspecialchars( xl('Deductible','e'), ENT_QUOTES) ?></td>
							<td width="60" class="left top" ><?php htmlspecialchars( xl('Takeback','e'), ENT_QUOTES) ?></td>
							<td width="60" class="left top" ><?php htmlspecialchars( xl('Follow Up','e'), ENT_QUOTES) ?></td>
							<td width="110" class="left top right" ><?php htmlspecialchars( xl('Follow Up Reason','e'), ENT_QUOTES) ?></td>
						  </tr>
						  <?php
						  }
						  ...
						  <tr class="text"  bgcolor='<?php echo $bgcolor; ?>'>
							...
							<td align="right" class="<?php echo $StringClass; ?>" ><input name="HiddenIns<?php echo $CountIndex; ?>" id="HiddenIns<?php echo $CountIndex; ?>"  value="<?php echo htmlspecialchars($Ins); ?>" type="hidden"/><?php 
								$frow['data_type']=1;
								$frow['list_id']='Payment_Ins';
								generate_print_field($frow, $Ins);
							 ?></td>
		  ...
		  <tr>
			<td colspan="13" align="left" >
				<table border="0" cellspacing="0" cellpadding="0" width="217" align="center">
				  ...
				  <tr>
					<td width="110"><a href="#" onClick="javascript:return ModifyPayments();"  class="css_button"><span><?php htmlspecialchars( xl('Modify Payments','e'), ENT_QUOTES);?></span></a>
					</td>
					<td width="107"><a href="#" onClick="javascript:return FinishPayments();"  class="css_button"><span><?php htmlspecialchars( xl('Finish Payments','e'), ENT_QUOTES);?></span></a>
					</td>
				  </tr>
				</table>
