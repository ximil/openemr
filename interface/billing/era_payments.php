...
//===============================================================================
  ...
  if ($_FILES['form_erafile']['size']) {
    ...
    if (is_file($erafullname)) {
      $alertmsg .= htmlspecialchars( xl("Warning: Set"), ENT_QUOTES).' '.$eraname.' '.htmlspecialchars( xl("was already uploaded"), ENT_QUOTES).' ';
      if (is_file($GLOBALS['OE_SITE_DIR'] . "/era/$eraname.html"))
	   {
        $Processed=1;
		$alertmsg .= htmlspecialchars( xl("and processed."), ENT_QUOTES).' ';
	   } 
      else
        $alertmsg .= htmlspecialchars( xl("but not yet processed."), ENT_QUOTES).' ';;
    }
...
function Validate()
 {
  ...
  if(document.getElementById('hidden_type_code').value=='')
   {
	alert("<?php htmlspecialchars( xl('Select Insurance, by typing','e'), ENT_QUOTES) ?>");
  ...
  if(document.getElementById('hidden_type_code').value!=document.getElementById('div_insurance_or_patient').innerHTML)
   {
	alert("<?php htmlspecialchars( xl('Take Insurance, from Drop Down','e'), ENT_QUOTES) ?>");
	document.getElementById('type_code').focus();
	return false;
   }
	top.restoreSession();
...
<table width="455" border="0"  cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td colspan="3" align="left">
		<ul class="tabNav"> 
		 ...
		 <li class='current'><a href='era_payments.php'><?php htmlspecialchars( xl('ERA Posting','e'), ENT_QUOTES) ?></a></li> 
		</ul>	</td>
  </tr>
  <tr>
    <td colspan="3" align="left" >
	...
	<table width="435" border="0" style="border:1px solid black" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
	    <td  align="left"></td>
		<td colspan="3"  align="left"><font class='title'><?php htmlspecialchars( xl('ERA','e'), ENT_QUOTES) ?></font></td>
	  </tr>
	  <tr height="5">
	    <td  align="left" ></td>
		<td colspan="3"  align="left" ></td>
   ...
   <input name="form_erafile" id="uploadedfile"  type="file" class="text" size="10" style="display:inline" /></td>
	  ...
	  <tr>
	   ...
	   <script>
	    ...
	    <td  align="left"  class="text"><input type='checkbox' name='form_without' value='1' <?php echo $_REQUEST['form_without']*1==1 || ($_REQUEST['form_without']*1==0 && !isset($_FILES['form_erafile'])) ? "checked" : '' ?>/> <?php htmlspecialchars( xl('Without Update','e'), ENT_QUOTES); ?></td>
	    </tr>
	  <tr>
	    <td  align="left"  class="text"></td>
	  ...
	  <tr>
	    <td  align="left"  class="text"></td>
	    <td  align="left"  class="text"><?php htmlspecialchars( xl('Insurance:','e'), ENT_QUOTES) ?></td>
  ...
  <tr>
    <td><a href="#" onClick="javascript:return Validate();" class="css_button"><span><?php htmlspecialchars( xl('Process ERA File','e'), ENT_QUOTES);?></span></a></td>
  </tr>
</table></td>
		</tr>
	  <tr height="5">
...
<input type="hidden" name="after_value" id="after_value" value="<?php echo htmlspecialchars($alertmsg);?>"/>
