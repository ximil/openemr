...
function CancelDistribute()
 {//Used in the cancel button.Helpful while cancelling the distribution.
	if(confirm("<?php htmlspecialchars( xl('Would you like to Cancel Distribution for this Patient?','e'), ENT_QUOTES) ?>"))
	 {
...
function PostPayments()
 {//Used in saving the allocation
	 ...
	 {
	  alert("<?php htmlspecialchars( xl('Fill the Row.','e'), ENT_QUOTES) ?>")
	  return false;
	 }
	...
	if(PostValue==1)
	 {
	  alert("<?php htmlspecialchars( xl('Cannot Post Payments.Undistributed is Negative.','e'), ENT_QUOTES) ?>")
	  return false;
	 }
	if(confirm("<?php htmlspecialchars( xl('Would you like to Post Payments?','e'), ENT_QUOTES) ?>"))
	 {
...
function FinishPayments()
 ...
 //After this is pressed a confirmation screen comes,where you can edit if needed.
	...
	if(PostValue==2)
	 {
		if(confirm("<?php htmlspecialchars( xl('Would you like to Post and Finish Payments?','e'), ENT_QUOTES) ?>"))
		 {
 ...
 {//Displays message after saving to master table.
  ...
  if(after_value=='distribute')
  ...
  else if(after_value=='new_payment')
   {
	...
	if(confirm("<?php htmlspecialchars( xl('Successfully Saved.Would you like to Allocate?','e'), ENT_QUOTES) ?>"))
	 {
		if(document.getElementById('TablePatientPortion'))
		 {
		  ...
		  <tr>
			<td colspan="13" align="left" >
				...
				<table border="0" cellspacing="0" cellpadding="0" width="267" align="center" id="AllocateButtons">
				  ...
				  <tr>
					<td width="100"><a href="#" onClick="javascript:return PostPayments();"  class="css_button"><span><?php htmlspecialchars( xl('Post Payments','e'), ENT_QUOTES);?></span></a></td>
					<td width="107"><a href="#" onClick="javascript:return FinishPayments();"  class="css_button"><span><?php htmlspecialchars( xl('Finish Payments','e'), ENT_QUOTES);?></span></a></td>
					<td width="60"><a href="#"  onClick="CancelDistribute()" class="css_button"><span><?php htmlspecialchars( xl('Cancel','e'), ENT_QUOTES);?></span></a></td>
				  </tr>
				</table>
				<?php
				 }//if($CountIndexBelow>0)
