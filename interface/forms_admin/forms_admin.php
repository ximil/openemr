...
?>
<option value="common_forms"  <?php if($speciality=='common_forms'){echo 'selected="selected"';}?>>Common Forms</option></select>
...
{
	$priority_category = sqlQuery("select priority, category, nickname,priority_list,allow_duplication,speciality from registry where id=".$registry['id']); 
	?>
	<div>
	<input type="hidden" value="<?php echo $registry['id'];?>" name="clorder[]">
	<table width="1000px" title="Drag and drop for changing menu order">
	<tr bgcolor="<?php echo $color?>" >
		...
		<?php
		$specialty_name=$priority_category['speciality'];
		if($specialty_name=='common_forms'){$specialty_name='Common';}
		else if(substr($specialty_name,0,11)=='Speciality_'){$specialty_name=ucwords(substr($specialty_name,11));}
		?>
	...
	?></div>
	<input type="hidden" name="slected_specialty" value="<?php echo $speciality;?>">
...
if ($bigdata != false)
{
	foreach ( $bigdata as $registry )
	{
		$key = array_search($registry['directory'].'::'.$registry['speciality'], $inDir) ;  /* returns integer or FALSE */
		unset($inDir[$key]);
	}
}
if(sizeof($inDir)>0)
...
while ($fname_spec=array_shift($inDir))
{
	...
	<tr>
		...
		<td bgcolor="<?php echo $color?>" width="30%">
	        <?php
                $form_title_file = @file($GLOBALS['srcdir']."/../interface/forms/$specl/$fname/info.txt");
                        if ($form_title_file)
		...
		<td bgcolor="<?php echo $color?>" width="10%"><?php
			if ($phpState == "PHP extracted")
				echo '<a class=link_submit href="./forms_admin.php?name=' . urlencode($fname) . '&method=register&specialty='.$specl.'&spec='.$speciality.'"'. $formtarget . '>' . xl('register') . '</a>';
			else
