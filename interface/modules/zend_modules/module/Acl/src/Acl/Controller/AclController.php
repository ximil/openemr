...
class AclController extends AbstractActionController
{
      ...
      /** Html Escape Function */
        ...
        foreach($array as $categoryId => $category) {
          ...
          if ($currentParent == $category['parent_id']) {
            ...
            $class="";
            echo '<li id="'.$category['parent_id']."-".$category['id'].'" value="'.$category['name'].'" '.$class.' ><div onclick="selectThis(\''.$category['parent_id'].'-'.$category['id'].'\');rebuild();" class="list">'.$escapeHtml($category['name'])."</div>";
    ...
    private function createUserGroups($id="user_group_",$visibility="",$dragabble="draggable",$li_class="")
    {
        ...
        foreach ($tempList as $groupID => $tempListRow) {
            $output_string .='<li '.$li_class.' id="li_'.$id.$tempListRow['group_id'].'-0" style="'.$visibility.'"><div class="'.$dragabble.'" id="'.$id.$tempListRow['group_id'].'-0" >' . $escapeHtml($tempListRow['group_name']).'</div>';
            if(!empty($tempListRow['items'])) {
              $output_string .='<ul>';   
              foreach ($tempListRow['items'] as $key => $itemRow){
                 $output_string .='<li '.$li_class.' id="li_'.$id.$itemRow['group_id'].'-'.$itemRow['user_id'].'" style="'.$visibility.'"><div class="'.$dragabble.'" id="'.$id.$itemRow['group_id'].'-'.$itemRow['user_id'].'">' . $escapeHtml($itemRow['display_name']) . '</div></li>';
              }
              $output_string .='</ul>';
            }
            $output_string .='</li>';
