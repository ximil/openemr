...
class EncounterccdadispatchTable extends AbstractTableGateway
{
    ...
    */
    ...
    {
        $query      = "select patient_data.*, l1.option_id AS race_code, l1.title as race_title, l2.option_id AS ethnicity_code, l2.title as ethnicity_title, l3.title as religion, l3.option_id as religion_code
                        from patient_data
	...
	$row        = $appTable->zQuery($query, array('race_ccda','ethnicity_ccda','religious_affiliation',$pid));
        ...
        foreach($row as $result){
            $patient_data = "<patient>
                ...
                <religion>".htmlspecialchars($result['religion'],ENT_QUOTES)."</religion>
    ...
    public function getInformationRecipient($pid,$encounter)
    {
    ...
    */
    public function getAllergies($pid,$encounter)
    {
        ...
        $allergies  = '';
        $query      = "SELECT l.id, l.title, l.begdate, l.enddate, lo.title AS observation, lo.option_id AS observation_code, lcode.list_code,SUBSTRING(lcode.list_code, 1, LOCATE(':',lcode.list_code)-1) AS code_type_real, SUBSTRING(lcode.list_code, LOCATE(':',lcode.list_code)+1) AS code
                from lists as l
        ...
        $allergies = "<allergies>";
        foreach($res as $row){
			$code = $code_text = $code_rx = $code_text_rx = $code_snomed = $code_text_snomed = '';
			
			if($row['code_type_real'] == 'Rxnorm'){
			$code_rx 		= explode(':',$row['code']);
			$code_text_rx 	= $row['title'];
			}
			else if($row['code_type_real'] == 'SNOMED'){
			$code_snomed 	  = explode(':',$row['code']);
			$code_text_snomed = lookup_code_descriptions($row['list_code']);
			}
			else{
			$code 			= explode(':',$row['code']);
			$code_text 		= lookup_code_descriptions($row['list_code']);
			}
            ...
            $allergies .= "<allergy>
				<id>".htmlspecialchars(base64_encode($_SESSION['site_id'].$row['id']),ENT_QUOTES)."</id>
				...
				<diagnosis_code>".htmlspecialchars(($code[0] ? $code[0] : 0),ENT_QUOTES)."</diagnosis_code>
				<diagnosis>".htmlspecialchars(($code_text ? $code_text : 'NULL'),ENT_QUOTES)."</diagnosis>
				<rxnorm_code>".htmlspecialchars(($code_rx[0] ? $code_rx[0] : 0),ENT_QUOTES)."</rxnorm_code>
				<rxnorm_code_text>".htmlspecialchars(($code_text_rx ? $code_text_rx : 'NULL'),ENT_QUOTES)."</rxnorm_code_text>
				<snomed_code>".htmlspecialchars(($code_snomed[0] ? $code_snomed[0] : 0),ENT_QUOTES)."</snomed_code>
				<snomed_code_text>".htmlspecialchars(($code_text_snomed ? $code_text_snomed : 'NULL'),ENT_QUOTES)."</snomed_code_text>
				<status>".($active ? $active : 'NULL')."</status>
				<status_code>".($status_code ? $status_code : 0)."</status_code>
				<outcome>".htmlspecialchars(($row['observation'] ? $row['observation'] : 'NULL'),ENT_QUOTES)."</outcome>
    ...
    {
        $medications    = '';
        $query      = "select l.id, l.date_added, l.drug, l.dosage, l.quantity, l.size, l.substitute, l.drug_info_erx, l.active, l.route as route_code,
		l.rxnorm_drugcode, l.erx_diagnosis, l.erx_diagnosis_name, l1.title as unit,l2.title as form, l3.title as route, l4.title as `interval`,
		u.fname, u.lname, u.mname, u.npi, u.street, u.streetb, u.city, u.state, u.zip, u.phonew1
        from prescriptions as l
        left join list_options as l1 on l1.option_id=form AND l1.list_id = ?
        left join list_options as l2 on l2.option_id=form AND l2.list_id = ?
        left join list_options as l3 on l3.option_id=form AND l3.list_id = ?
        left join list_options as l4 on l4.option_id=form AND l4.list_id = ?
		...
		$row['rxnorm_drugcode'] = $this->generate_code($row['drug']);
            ...
            if($row_currentMed['size'] > 0)
                $unit = $row['size']." ".$row['unit']." ";
            $str = $unit." ".$row['dosage']." ".$row['form']." ".$row['route']." ".$row['interval'];
            ...
            $medications .= "<medication>
				...
				<size>".htmlspecialchars($row['size'],ENT_QUOTES)."</size>
				<unit>".htmlspecialchars($row['unit'],ENT_QUOTES)."</unit>
				...
				<interval>".htmlspecialchars($row['interval'],ENT_QUOTES)."</interval>
                <start_date>".htmlspecialchars($row['date_added'],ENT_QUOTES)."</start_date>
				<end_date>".htmlspecialchars('00000000',ENT_QUOTES)."</end_date>
                <status>".$active."</status>
                <indications>".htmlspecialchars(($row['erx_diagnosis_name'] ? $row['erx_diagnosis_name'] : 'NULL'),ENT_QUOTES)."</indications>
				<indications_code>".htmlspecialchars(($row['erx_diagnosis'] ? $row['erx_diagnosis'] : 0),ENT_QUOTES)."</indications_code>
    ...
    {
        $problem_lists  = '';
        $query      = "select l.*, lo.title as observation, lo.option_id as observation_code, lcode.snomed_code AS code, sct.FullySpecifiedName AS code_text
		from lists AS l
		LEFT JOIN list_codes AS lcode ON l.id=lcode.list_id
		LEFT JOIN sct_concepts AS sct ON sct.ConceptId=lcode.snomed_code
        left join list_options as lo on lo.option_id = l.outcome AND lo.list_id = ?
        where l.type = ? and l.pid = ?";
        $appTable   = new ApplicationTable();
		$res        = $appTable->zQuery($query, array('severity_ccda','medical_problem',$pid));
        ...
        foreach($res as $row){
			...
			//Active - 55561003 	Inactive - 73425007
			if($end_date){
				$status 		= 'InActive';
				$status_code    = '73425007';
			}
            ...
            $problem_lists .= "<problem>
                ...
                <observation>".htmlspecialchars(($observation ? $observation : 'NULL'),ENT_QUOTES)."</observation>
    ...
    {
	$immunizations = '';
	$query 	    = "SELECT im.*, cd.code_text, DATE_FORMAT(administered_date,'%b %Y') AS administered_date,
		    ...
		    LEFT JOIN list_options AS lo ON lo.list_id = 'immunization_route' AND lo.option_id = im.route
	...
	foreach($res as $row){
		...
		<route_of_administration>".htmlspecialchars($row['route_of_administration'],ENT_QUOTES)."</route_of_administration>
		<route_code>".htmlspecialchars($row['route'],ENT_QUOTES)."</route_code>
    ...
    {        
        $procedure  = '';
        $query      = "select b.id, b.date as proc_date, c.code_text, c.code, fe.date,
	...
	LEFT JOIN facility AS f ON f.id = fe.facility_id
        ...
        $appTable   = new ApplicationTable();
	$res        = $appTable->zQuery($query, array('CPT4',$pid,1));
    ...
    public function getResults($pid,$encounter)
    {
        ...
        $query = "SELECT psr.result_value AS result_value, psr.units, psr.range, psr.order_title, psr.subtest_code as result_code,
	    psr.subtest_desc as result_desc, psr.code_suffix AS test_code, po.date_ordered, psr.result_time AS result_time, psr.abnormal_flag
	...
	foreach($res as $row){
	    ...
	    $results_list[$row['test_code']]['subtest'][$row['result_code']]['result_code'] = $row['result_code'];
    ...
    */    
	...
	$query 	 = "SELECT fe.date, fe.encounter,
	    f.id as fid, f.name, f.phone, f.street as fstreet, f.city as fcity, f.state as fstate, f.postal_code as fzip, f.country_code, f.phone as fphone,
	    u.fname, u.mname, u.lname, u.npi, u.street, u.city, u.state, u.zip, u.phonew1, cat.pc_catname, lo.title, lo.option_id
	...
	foreach($res as $row){
		...
		<physician_type_code>".htmlspecialchars($row['option_id'],ENT_QUOTES)."</physician_type_code>
    ...
    */
    ...
    {
        $vitals = '';
        $query  = "SELECT DATE(fe.date) AS date, fv.id, temperature, bpd, bps, head_circ, pulse, height, oxygen_saturation, weight FROM forms AS f
                JOIN form_encounter AS fe ON fe.encounter = f.encounter AND fe.pid = f.pid
                JOIN form_vitals AS fv ON fv.id = f.form_id
                WHERE f.pid = ? AND f.formdir = 'vitals'
        ...
        foreach($res as $row){
            ...
            $social_history .= "<history_element>
				...
				<sha_extension>".htmlspecialchars("9b56c25d-9104-45ee-9fa4-e0f3afaa01c1", ENT_QUOTES)."</sha_extension>
                                ...
                                <status>".htmlspecialchars(($snomeds[$tobacco[1]] ? $snomeds[$tobacco[1]] : 0),ENT_QUOTES)."</status>
            ...
            $social_history .= "<history_element>
				...
				<sha_extension>".htmlspecialchars("37f76c51-6411-4e1d-8a37-957fd49d2cef", ENT_QUOTES)."</sha_extension>
                                ...
                                <status>".htmlspecialchars(($arr_status[$alcohol[1]] ? $arr_status[$alcohol[1]] : 'NULL'),ENT_QUOTES)."</status>
    ...
    */
    ...
    {
        $age = 0;
        $query      = "select ROUND(DATEDIFF(NOW(),DOB)/365.25) AS age from patient_data where pid= ? ";
        $appTable   = new ApplicationTable();
	$res        = $appTable->zQuery($query, array($pid));
        ...
        foreach($formTables as $formTables_details){
            ...
            if($formTables_details[0] == 1){//Fetching the values from an HTML form
                ...
                if(!$formTables_details[1]){//Fetching the complete LBF
                    foreach($form_ids as $row){
                        foreach($row as $key => $value){
                            //if(file_exists(dirname(__FILE__).'/../../forms/LBF/report.php')){
                            //    include_once(dirname(__FILE__).'/../../forms/LBF/report.php');
                            //    echo "<table><tr><td>";
                            //    call_user_func("lbf_report", $pid, $encounter, 2, $form_id, $formTables_details[2]);
                            //    echo "</td></tr></table>";
                            //}
                        }
                    }
			...
			if(count($matches) == 0) $row['field_value'] .= ". ";
                ...
                foreach($result as $row_folders){
                    $r = $this->retrieve_action($pid, $row_folders['document_id']);
                    $res[0][$count_folder][0] = $r;
    ...
    */
    public function logCCDA($pid, $encounter, $content, $time, $status, $user_id, $view = 0, $transfer = 0)
    {
		...
		if($GLOBALS['document_storage_method']==1){
			...
			$docid 	= preg_replace('/[^0-9a-zA-Z_]/','',uniqid(rand().'_',true));
			$json 	= json_encode(base64_encode($content));
			$data 	= array($db, $docid, $pid, $encounter, 'xml', $json);
			$resp 	= $couch->check_saveDOC($data);
			if(!$resp->id || !$resp->rev){
				$data 	= array($db, $docid, $pid, $encounter);
				$resp 	= $couch->retrieve_doc($data);
				$docid 	= $resp->_id;
				$revid 	= $resp->_rev;
			}
        ...
        $appTable   = new ApplicationTable();
        $result     = $appTable->zQuery($query, array($pid, $encounter, $file_path, $time, $status, $user_id, $docid, $revid, $view, $transfer));
        return;
    }
