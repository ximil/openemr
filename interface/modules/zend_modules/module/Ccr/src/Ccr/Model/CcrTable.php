...
class CcrTable extends AbstractTableGateway
{
          ...
          for($i=0;$i<count($val);$i++){
            if($val[$i] == 'insert'){
              if(substr($key,0,-4) == 'lists1'){
                ...
                $appTable->zQuery($query, array($data['pid'], $data['lists1-diagnosis'][$i], $activity, $data['lists1-title'][$i], $data['lists1-date'][$i], 'medical_problem'));
              }
              elseif(substr($key,0,-4) == 'lists2'){
                $query = "INSERT INTO lists (pid, date, type, title, diagnosis, reaction) VALUES (?,?,?,?,?,?)";
                $appTable->zQuery($query, array($data['pid'], $data['lists2-date'][$i], $data['lists2-type'][$i], $data['lists2-title'][$i], $data['lists2-diagnosis'][$i], $data['lists2-reaction'][$i]));
              }
              elseif(substr($key,0,-4) == 'prescriptions'){
                ...
                $appTable->zQuery($query, array($data['pid'], $data['prescriptions-date_added'][$i],$active, $data['prescriptions-drug'][$i], $data['prescriptions-size'][$i], $data['prescriptions-form'][$i],$data['prescriptions-quantity'][$i]));
              }
              elseif(substr($key,0,-4) == 'immunizations'){
                $query = "INSERT INTO immunizations (patient_id, administered_date, note) VALUES (?,?,?)";
                $appTable->zQuery($query, array($data['pid'], $data['immunizations-administered_date'][$i], $data['immunizations-note'][$i]));
              }
            }
            elseif($val[$i] == 'update'){
              if(substr($key,0,-4) == 'lists1'){
  ...
  */
  ...
  /*
  * Retrive the CCR xml document from documents table
  *
  * @param    document_id     Integer     id from documents table
  * @return   content         String      Content of CCR xml file
  */
