          ...
          for($i=0;$i<count($val);$i++){
            if($val[$i] == 'insert'){
              if(substr($key,0,-4) == 'lists1'){
                ...
                $appTable->zQuery($query, array($data['pid'], $data['lists1-diagnosis'][$i], $activity, $data['lists1-title'][$i], \Application\Model\ApplicationTable::fixDate($data['lists1-date'][$i], 'yyyy-mm-dd', $this->date_format), 'medical_problem'));
              }
              elseif(substr($key,0,-4) == 'lists2'){
                $query = "INSERT INTO lists (pid, date, type, title, diagnosis, reaction) VALUES (?,?,?,?,?,?)";
                $appTable->zQuery($query, array($data['pid'], \Application\Model\ApplicationTable::fixDate($data['lists2-date'][$i], 'yyyy-mm-dd', $this->date_format), $data['lists2-type'][$i], $data['lists2-title'][$i], $data['lists2-diagnosis'][$i], $data['lists2-reaction'][$i]));
              }
              elseif(substr($key,0,-4) == 'prescriptions'){
                ...
                $appTable->zQuery($query, array($data['pid'], \Application\Model\ApplicationTable::fixDate($data['prescriptions-date_added'][$i], 'yyyy-mm-dd', $this->date_format),$active, $data['prescriptions-drug'][$i], $data['prescriptions-size'][$i], $data['prescriptions-form'][$i],$data['prescriptions-quantity'][$i]));
              }
              elseif(substr($key,0,-4) == 'immunizations'){
                $query = "INSERT INTO immunizations (patient_id, administered_date, note) VALUES (?,?,?)";
                $appTable->zQuery($query, array($data['pid'], \Application\Model\ApplicationTable::fixDate($data['immunizations-administered_date'][$i], 'yyyy-mm-dd', $this->date_format), $data['immunizations-note'][$i]));
              }
              elseif(substr($key,0,-4) == 'procedure_result'){
                //$query = "INSERT INTO procedure_type (name) VALUES (?)";
                //sqlQuery($query,array($data['procedure_type-name'][$i]));
                //$query = "INSERT INTO procedure_result (date,result,abnormal) VALUES (?,?,?)";
                //sqlQuery($query,array($data['procedure_result-date'][$i],$active,$data['procedure_result-abnormal'][$i]));
              }
            }
            elseif($val[$i] == 'update'){
              if(substr($key,0,-4) == 'lists1'){
