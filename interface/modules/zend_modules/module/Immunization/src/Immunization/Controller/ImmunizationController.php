    ...
    */
    public function reportAction()
    {
        ...
        if(isset($data['hl7button'])){
            $form_code        =   isset($data['codes']) ? $data['codes'] : Array();
            $from_date       =   $request->getPost('from_date', null) ? $request->getPost('from_date', null) : date('Y-m-d',strtotime(date('Ymd')) - (86400*7));
            $to_date         =   $request->getPost('to_date', null) ? $request->getPost('to_date', null) : date('Y-m-d');
            $form_get_hl7     =   'true'; 
            //pagination
            $results          =   $request->getPost('form_results', 100);
            $results          =   ($results > 0) ? $results : 100;
