...
class InstModuleTable
{
    ...
    public function register($directory,$rel_path,$state=0, $base = "custom_modules" )
    {
      ...
      if($arr['total'] > 0){
        ...
        $result = $this->applicationTable->zQuery($sql_max_id);
        $arr = $sec_result->current();
    ...
    public function getInstalledModules()
    {
    	$all = array();
			...
			$res =  $this->applicationTable->zQuery($sql,array("1"));
    ...
    public function getActiveHooks($mod_id)
    {
      ...
      $res		= $this->applicationTable->zQuery($sql,array("3", "1", $mod_id));
      foreach($res as $row) {
        $mod = new InstModule();
        $mod -> exchangeArray($row);
        array_push($all,$mod);        
    ...
    {  
      ...
      $res	= $this->applicationTable->zQuery($sql, array($mod_id, "3", $hookId));
			foreach($res as $row){
					$modArr	= $row;
			}			
      if($modArr['obj_name'] <> ""){
		...
		public function insertAclSections($acl_data,$mod_dir,$module_id)
    {
      ...
      if(!$exists){
          ...
          $result = $obj->zQuery($sql,array($module_id,1,$mod_dir,$mod_dir));
      }
		}
    ...
    {				
      if($modId){
        ...
        $this->applicationTable->zQuery($sql, array($modId, "3", $hookId, $hookTitle, $hookPath));			
      }
    }
		...
		public function deleteModuleHookSettings($modId)
    {
          ...
          $sql = "DELETE FROM modules_settings WHERE mod_id = ? AND fld_type = ?";
						$this->applicationTable->zQuery($sql, array($modId, "3"));			
				}
    }
