...
class InstModuleTable
{
    ...
    public function register($directory,$rel_path,$state=0, $base = "custom_modules" )
    {
      ...
      $sec_result = $this->applicationTable->sqlQuery($sec_count);
      $arr = $sec_result->current();
      if($arr['total'] > 0){
        $sql_max_id = "SELECT MAX(section_id) as max_id FROM module_acl_sections";
        $result = $this->applicationTable->sqlQuery($sql_max_id);
        $arr = $sec_result->current();
    ...
    function getRegistryEntry( $id, $cols = "" )
    {
      ...
      $results   = $this->applicationTable->sqlQuery($sql, array($id));
    ...
    public function getSettings($type,$mod_id)
    {
      ...
      $all = array();
    	...
    	$res = $this->applicationTable->sqlQuery($sql, array($mod_id, $type));
    	if($res){
    ...
    public function getOemrUserGroup()
    {
      $all = array();
    	$sql = "SELECT * FROM gacl_aro_groups AS gag 
                          ...
                          GROUP BY id ";
    	$res = $this->applicationTable->sqlQuery($sql);
    	if($res){
    		foreach($res as $key => $m) {
          $mod = new InstModule();
          $mod -> exchangeArray($m);
    ...
    public function getTabSettings($mod_id)
    {
      $all = array();
    	$sql = "SELECT fld_type,COUNT(*) AS cnt  
                    ...
                    GROUP BY fld_type 
                    ORDER BY fld_type ";
    	$res = $this->applicationTable->sqlQuery($sql,array($mod_id));
    	if($res){
    		foreach($res as $key => $m) {
    ...
    public function getActiveACL($mod_id)
    {
      ...
      $sql = "SELECT mod_directory FROM modules WHERE mod_id=?";
      $result = $this->applicationTable->sqlQuery($sql, array($mod_id));
      $Section = $result->current();
      $aco = "modules_" . $Section['mod_directory'];
      
      $sql = "SELECT * FROM gacl_aco_map WHERE section_value=?";
      $MapRes = $this->applicationTable->sqlQuery($sql, array($aco));
      foreach ($MapRes as $key => $MapRow) {
        ...
        $aroRes = $this->applicationTable->sqlQuery($sqlSelect, array($MapRow['acl_id']));
        $i=0;
    ...
    public function SaveConfigurations($post)
    {
      foreach($post as $aco=>$acoArray){
        ...
        foreach($acoArray as $aroKey => $aro){
          $sql    = "SELECT acl_id FROM gacl_aco_map WHERE section_value=? AND value=?";
          $result = $this->applicationTable->sqlQuery($sql, array($acoSection, $acoValue));
          $ACLARR = $result->current();
          if($ACLARR['acl_id']){
            $aclSeq = $ACLARR['acl_id'];
          } else {
            $sqlUpdate = "UPDATE gacl_acl_seq SET id=LAST_INSERT_ID(id+1)";
            $this->applicationTable->sqlQuery($sqlUpdate);
            $sqlSelect    = "SELECT id FROM gacl_acl_seq";
            $resultSelect = $this->applicationTable->sqlQuery($sqlSelect);
            $aclSeqArr    = $resultSelect->current();
            $aclSeq       = $aclSeqArr['id'];
            $sqlInsert = "INSERT INTO gacl_acl (id,section_value,allow,enabled,return_value,note) VALUES(?,?,1,1,?,?)";
            $this->applicationTable->sqlQuery($sqlInsert, array($aclSeq, 'user', '', ''));
            $sqlInsert = "INSERT INTO gacl_aco_map (acl_id,section_value,value) VALUES (?,?,?)";
            $this->applicationTable->sqlQuery($sqlInsert, array($aclSeq, $acoSection, $acoValue));
          }
          $sqlInsert = "INSERT INTO gacl_aro_map (acl_id,section_value,value) VALUES (?,?,?)";
          $this->applicationTable->sqlQuery($sqlInsert, array($aclSeq, 'users', $aro));
        }
      }
    }
    ...
    public function getHookStatus($modId,$hookId,$hangerId)
    {
      if($modId && $hookId && $hangerId){	
        $sql = "select * FROM modules_hooks_settings 
                          ...
                          AND enabled_hooks = ? 
                          AND attached_to = ? ";
        $res	= $this->applicationTable->sqlQuery($sql, array($modId, $hookId, $hangerId));
        foreach($res as $row){
            $modArr	= $row;
        }
    ...
    public function DeleteAcl($post)
    {
      if($post['aclID'] && $post['user']){
        ...
        $this->applicationTable->sqlQuery($sql, array($post['aclID'], $post['user']));
      }
    }
    ...
    public function saveHooks($modId,$hookId,$hangerId)
    {
        ...
        $sql = "INSERT INTO modules_hooks_settings(mod_id, enabled_hooks, attached_to) VALUES (?,?,?) ";
				$this->applicationTable->sqlQuery($sql, array($modId, $hookId, $hangerId));			
			}
    }
    ...
    public function getModuleStatusByDirectoryName($moduleDir)
    {
      ...
      $res	= $this->applicationTable->sqlQuery($sql, array(trim($moduleDir)));
				foreach($res as $row) {
						$check	= $row;
				}
    ...
    {  
			$sql = "SELECT obj_name FROM modules_settings WHERE mod_id = ? AND fld_type = ? AND obj_name = ? ";
      $res	= $this->applicationTable->sqlQuery($sql, array($mod_id, "3", $hookId));
			foreach($res as $row){
					$modArr	= $row;
			}			
      if($modArr['obj_name'] <> ""){
    ...
    {				
      if($modId){
        $sql = "INSERT INTO modules_settings(mod_id, fld_type, obj_name, menu_name, path) VALUES (?,?,?,?,?) ";
        $this->applicationTable->sqlQuery($sql, array($modId, "3", $hookId, $hookTitle, $hookPath));			
      }
    }
    ...
    {
				if($modId){
          $sql = "DELETE FROM modules_settings WHERE mod_id = ? AND fld_type = ?";
						$this->applicationTable->sqlQuery($sql, array($modId, "3"));			
				}
    }
