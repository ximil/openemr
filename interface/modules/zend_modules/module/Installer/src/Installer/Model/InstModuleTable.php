      ...
      foreach($post as $aco=>$acoArray){
        ...
        foreach($acoArray as $aroKey => $aro){
          ...
          if($ACLARR['acl_id']){
            ...
            $aclSeq       = $aclSeqArr['id'];
            $sqlInsert = "INSERT INTO gacl_acl (id,section_value,allow,enabled,return_value,note) VALUES(?,'user',1,1,?,?)";
            $this->applicationTable->zQuery($sqlInsert, array($aclSeq, '', ''));
          ...
          $sqlInsert = "INSERT INTO gacl_aro_map (acl_id,section_value,value) VALUES (?,'users',?)";
          $this->applicationTable->zQuery($sqlInsert, array($aclSeq, $aro));
        }
      }
    }
