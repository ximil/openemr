...
class SyndromicsurveillanceController extends AbstractActionController
{
    ...
    */
    public function indexAction()
    {
        ...
        $fromDate       = $request->getPost('form_date_from', null) ? $request->getPost('form_date_from', null) : date('m/d/Y',strtotime(date('Ymd')) - (86400*7));
        $toDate         = $request->getPost('form_date_to', null) ? $request->getPost('form_date_to', null) : date('m/d/Y');
        $fromDate       = $this->getSyndromicsurveillanceTable()->convert_to_yyyymmdd($fromDate);
        $toDate         = $this->getSyndromicsurveillanceTable()->convert_to_yyyymmdd($toDate);
        ...
        $params     = array(
                        'form_date_from'    => $this->getSyndromicsurveillanceTable()->date_format($fromDate),
                        'form_date_to'      => $this->getSyndromicsurveillanceTable()->date_format($toDate),
