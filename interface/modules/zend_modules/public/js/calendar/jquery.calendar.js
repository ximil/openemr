/**
  * @description {Class} wdCalendar v1.4.1
  * This is the main class of wdCalendar.
  * -Remake Piotr SiekierzyA?ski
  * - add option: intervalTime[integer] (default: 30 minutes), hourFormat[integer 12/24] (default: 24)
  * - add live change red arrow on timeline and red line on current day
  
  *Edit CSS (remove round via HTML, set via CSS):
...
(function($) {
    ...
    $.fn.bcalendar = function(option) {
        var def = {
            ...
            intervalTime: 30, 
            /**
             * @description {Config} hourFormat  
             * {Integer} Set hour format. 24 - 24h, 12 - 12h 
             */
            hourFormat: 24
        };
        ...
        var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};\" title=\"${title}\" class=\"chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DL style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; HEIGHT: ${height}px; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\"><DT style=\"BACKGROUND-COLOR:${bgcolor2}\">${starttime} - ${endtime} ${icon}</DT><DD><SPAN>${content}</SPAN></DD><DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV></DL></DIV>";
        ...
        if (!option.height) {
			...
			window.onresize = function() {
				option.height = document.documentElement.clientHeight - 30;
				render();
				gridcontainer.height(option.height);
			}
		...
		//Detect other number for hour format.
        if (!/12|24/g.test(option.hourFormat)) {
            option.hourFormat = 24;
        }
        //
        gridcontainer.height(option.height);
        ...
        function render() {
            ...
            if (option.view == "day" || option.view == "week") {
                var $dvtec = $("#dvtec");
                if ($dvtec.length > 0) {
                    option.scoll = $dvtec.scrollTop(); //get scroll bar position
                }
            }
            switch (option.view) {
                case "day":
                ...
                case "month":
                    ...
                    break;
                case "agenda":
                    BuildAgendaView(showday, events, config);
                    break;
                default:
        ...
        //build agenda view
        function BuildAgendaView(showday, events, config) {
            gridcontainer.html('<div style="background-color:white;width:100%; height:100%;">test agenda '+showday+'<br>'+print_r(events).replace("\n",'<br>')+'</div>');
            html = null;
        }
        function closeCc() {
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            ht.push("<th width=\"",($.scrollbarWidth()-1),"\" rowspan=\"3\">&nbsp;</th>");
            ht.push("</tr>"); //end tr1;
            //2:          
            ht.push("<tr>");
            ht.push("<td class=\"wk-allday\" style='position: relative;'");

            if (dayarrs.length > 1) {
                ht.push(" colSpan='", dayarrs.length, "'");
            }
            //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
            ht.push("><div style='position: relative;'><div id=\"weekViewAllDaywkbg\" style='position: absolute; width: 100%; height: 100%; z-index: 0;'><table class=\"st-grid\" cellpadding=\"0\" cellspacing=\"0\" style='height: 100%;'><tbody>");
            ht.push("<tr>");
                for (var i = 0; i < dayarrs.length; i++) {
					var istoday = dateFormat.call(dayarrs[i].date, "yyyyMMdd") == dateFormat.call(new Date(), "yyyyMMdd");
					// Today
					if (istoday) {
						ht.push("<td style=\"width:3px;\"></td><td abbr='", dateFormat.call(dayarrs[i].date, "M/d/yyyy"), "' class=\"tg-today\"></td>");
					}
					else
					{
						ht.push("<td style=\"width:3px;\"></td><td abbr='", dateFormat.call(dayarrs[i].date, "M/d/yyyy"), "'></td>");
					}
                }
            ht.push("</tr>");
            ht.push("</table></div>");
        ...
        function BuildDayScollEventContainer(ht, dayarrs, events) {
			...
			if (typeof realodTime == "undefined"){
				realodTime = window.setInterval(function(){
					var _now = new Date(); var _h = _now.getHours(); var _m = _now.getMinutes();
					var _mhh = gP(_h, _m);
					var format = dateFormat.call(new Date(), "M/d/yyyy");
					if($('#tgTable td[abbr="'+format+'"] div.tg-today').length == 0){
						$('#weekViewAllDaywkbg td.tg-today').removeClass('tg-today');
						if($('#tgTable td[abbr="'+format+'"]').length > 0){
							if($('#tgTable div.tg-today').length > 0){
								$('#tgTable div.tg-today').prependTo($('#tgTable td[abbr="'+format+'"]'));
							}
							else
							{
								$('#tgTable td[abbr="'+format+'"]').prepend('<div style="margin-bottom: -'+height_col+'px; height:'+height_col+'px" class="tg-today"></div>');
							}
							if($('#tgTable #tgnowmarker').length > 0){
								$('#tgTable #tgnowmarker').prependTo($('#tgTable td[abbr="'+format+'"] div.tg-col-overlaywrapper'));
							}
							else
							{
								$('#tgTable td[abbr="'+format+'"] div.tg-col-overlaywrapper').prepend('<div id="tgnowmarker" class="tg-hourmarker tg-nowmarker" style="left:0px;top:'+_mhh+'px"></div>');
							}
							$('#weekViewAllDaywkbg td[abbr="'+format+'"]').addClass('tg-today');
						}
						else
						{
							$('#tgTable div.tg-today').remove();
							$('#tgTable #tgnowmarker').remove();
						}
					}
					$('#tgnowmarker').css('top', _mhh + 'px');
					var _mHg = _mhh - 4;
					$('#tgnowptr').css('top', _mHg + 'px');
				},15000);
			}
        }
        ...
        function getTitle(event) {			
			...
			ageshow = (event[11] != undefined && event[11] != "") ? event[11] : "";
			dobshow = (event[12] != undefined && event[12] != "") ? event[12] : "";
			// End OEMR
			if(option.hourFormat == 12){
				var st = et = a = '';
				var th_tmp , tm_tmp;
				st = dateFormat.call(event[2], startformat);
				if(/HH:mm/g.test(startformat)){
					th_tmp = dateFormat.call(event[2], 'HH');
					tm_tmp = dateFormat.call(event[2], 'mm');
					a = (th_tmp<12) ? 'AM' : 'PM';
					th_tmp = th_tmp%12;
					if(th_tmp == '0'){th_tmp = '12';}	
					st = st.replace(dateFormat.call(event[2], 'HH:mm'),th_tmp+':'+tm_tmp+' '+a);
				}
				et = dateFormat.call(event[3], endformat);
				if(/HH:mm/g.test(endformat)){
					th_tmp = dateFormat.call(event[3], 'HH');
					tm_tmp = dateFormat.call(event[3], 'mm');
					a = (th_tmp<12) ? 'AM' : 'PM';
					th_tmp = th_tmp%12;
					if(th_tmp == '0'){th_tmp = '12';}	
					et = et.replace(dateFormat.call(event[3], 'HH:mm'),th_tmp+':'+tm_tmp+' '+a);
				}
				timeshow = st + " - " + et;		
			}
			else
			...
			if(eventshow != undefined && $.trim(eventshow) != ""){
				ret.push($.browser.mozilla?"":"\r\n",i18n.xgcalendar.event + ": ", $.trim(eventshow));
            }
			
			if(event[9] != undefined && $.trim(event[9]) != ""){
				ret.push($.browser.mozilla?"":"\r\n",i18n.xgcalendar.location + ": ", $.trim(event[9]));
            }
        ...
        function BuildDayEvent(theme, e, index) {
			...
			if((dateFormat.call(new Date(e.event[2]), "yyyyMMdd")) != (dateFormat.call(new Date(e.event[3]), "yyyyMMdd"))){
				var eP = gP(24, 0);
			}
			else
			{
			...
			var sa = ea = '';
			if(option.hourFormat == 12){
				sa = (e.st.hour<12) ? ' AM' : ' PM';
				e.st.hour = e.st.hour%12;
				if(e.st.hour == '0'){e.st.hour = '12';}	
				ea = (e.et.hour<12) ? ' AM' : ' PM';
				e.et.hour = e.et.hour%12;
				if(e.et.hour == '0'){e.et.hour = '12';}		
			}
			else
			{
				e.st.hour = pZero(e.st.hour);
				e.et.hour = pZero(e.et.hour);
			}
            //p.starttime = e.st.hour + ":" + pZero(e.st.minute) + sa;
            //p.endtime = e.et.hour + ":" + pZero(e.et.minute) + ea;
			
			// Start OEMR
			var title = 'Click to Edit ... !';
			p.starttime = '<span title="' + title + '">' + "<img src='http://192.168.1.241/openemr_calendar/src/Plugins/pencil.png' >" + e.st.hour + ":" + pZero(e.st.minute) + sa;
			var title = getTitle(e.event);
            p.endtime = e.et.hour + ":" + pZero(e.et.minute) + ea + " " + "</span><img src='http://192.168.1.241/openemr_calendar/src/Plugins/user.png'>"+'<a href="javascript(0);" id="a_'+e.event[0]+'" title="' + title + '" onClick="getPatient(\'' + e.event[13] + '\');">' + e.event[1] + "</a>";
			// End OEMR
        ...
        function BuildMonthDayEvent(e, cday, length) {
            ...
            if (!e.allday && !sf) {
				if(option.hourFormat == 12){
					a = (e.st.hour<12) ? ' AM' : ' PM';
					e.st.hour = e.st.hour%12;
					if(e.st.hour == '00'){e.st.hour = '12';}
				}
				else
				{
					e.st.hour = pZero(e.st.hour);
				}
                //cen = e.st.hour + ":" + pZero(e.st.minute) + a + " " + e.event[1];
				// Start OEMR
				var title = getTitle(e.event);
				cen = "<img src='http://192.168.1.241/openemr_calendar/src/Plugins/pencil.png'>" + pZero(e.st.hour) + ":" + pZero(e.st.minute) + " " + "<img src='http://192.168.1.241/openemr_calendar/src/Plugins/user.png'>" + 
				'<a href="javascript(0);" id="a_' + e.event[0] + '" title="' + title + '" onClick="getPatient(\'' + e.event[13] + '\');">' + e.event[1] + "</a>";
				// End OEMR
            }
            else {
                cen = e.event[1];
        ...
        function buildtempdayevent(sh, sm, eh, em, h, title, w, resize, thindex) {
            ...
            var newtemp = Tp(__SCOLLEVENTTEMP, {
                ...
                bgcolor1: theme[2],
                data: "",
                starttime: [sh, pZero(sm) + sa].join(":"),
                endtime: [eh, pZero(em) + ea].join(":"),
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (buddle.length == 0) {
                ...
                calbutton.click(function(e) {
					...
					if((new Date(datestart).getTime() / 1000) > (new Date(dateend).getTime() / 1000)){
						dateend = dateFormat.call(new Date(new Date(dateend).getTime() + (24 * 60 * 60 * 1000)), "M/d/yyyy HH:mm");
					}
                    ...
                    if (option.quickAddHandler && $.isFunction(option.quickAddHandler)) {
                        ...
                        var diff = DateDiff("d", sd, ed);
						if((diff == 1) && (dateFormat.call(ed, "HH:mm") == '00:00')){
							diff = 0;
						}
        ...
        function dragMove(e) {
            if (_dragdata) {//document.title = e.pageX;
                ...
                switch (d.type) {
                    ...
                    case 4: // event moving
                        if (d.cpwrap) {
							...
							window.document.title = 'Moving: '+start+' '+end;
							if((new Date(start).getTime() / 1000) > (new Date(end).getTime() / 1000)){
								end = new Date(+new Date(end) + 1000*60*60*24);
							}
                    ...
                    case 5: //Resize
                        if (d.cpwrap) {
							...
							window.document.title = 'Resize: '+start+' '+end;
							if((new Date(start).getTime() / 1000) > (new Date(end).getTime() / 1000)){
								end = new Date(+new Date(end) + 1000*60*60*24);
							}
                            if (start.getTime() == d.data[2].getTime() && end.getTime() == d.data[3].getTime()) {
