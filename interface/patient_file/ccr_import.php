...
if($_REQUEST["ccr_ajax"] == "yes"){
  ...
  if($storagemethod == 1){
    ...
    $tmpxml = $GLOBALS['OE_SITE_DIR'].'/documents/temp/couch_xml_'.date("YmdHis").$d->get_url_file();
    $fh = fopen($tmpxml,"w");
  ...
  if(file_exists($file)){
    ...
    unlink($file);
    echo xlt('Successfully Imported the details').". ".xlt('Please approve the patient from the Pending Approval Screen').".";
  }else{
...
<html>
<head>
<title><?php echo xlt('Import CCR XML');?></title>
...
<table style="width:85%;font-size:14px;" >
  <tr>
    <td>1.</td>
    <td><?php echo xlt('To upload CCR document of already existing patient use Patient Summary Screen').'->'.xlt('Documents').', '.xlt('For CCR document of a new patient use Miscellanous').'->'.xlt('New Documents screen').'.'; ?></td>
  </tr>
  <tr>
    <td>2.</td>
  ...
  <tr>
    <td>4.</td>
    <td><?php echo xlt('Approve the patient from Patient').'/'.xlt('Client').'->'.xlt('Import').'->'.xlt('Pending Approval').'.'; ?></td>
  </tr>
</table>
</center>
</form>
