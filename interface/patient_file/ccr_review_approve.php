								...
								if($res_pd['field_name'] != 'lname' && $res_pd['field_name'] != 'fname' && $res_pd['field_name'] != 'DOB'){
									...
									if($res_pd['field_name'] == 'sex'){
										echo "<td>".($patient_data[$res_pd['field_name']] ? text($patient_data[$res_pd['field_name']]): text($res_pd['field_name']))."</td>
											<td><select name='".$res_pd['table_name']."-".$res_pd['field_name']."' style='width:150px;' ><option value='Male' ".($res_pd['field_value'] == 'Male' ? 'selected' : '' )." >".xlt('Male')."</option>
											<option value='Female' ".($res_pd['field_value'] == 'Female' ? 'selected' : '' )." >".xlt('Female')."</option></select><span style='color:red;padding-left:25px;' >".text($query_oldpd['val'])."</span></td>
											<td><select name='".$res_pd['table_name']."-".$res_pd['field_name']."-sel'><option value='ignore' >".xlt('Ignore')."</option><option value='update' >".xlt('Update')."</option></select></td>";
									}else{
										echo "<td>".($patient_data[$res_pd['field_name']] ? text($patient_data[$res_pd['field_name']]): text($res_pd['field_name']))."</td>
											<td><input type='text' name='".$res_pd['table_name']."-".$res_pd['field_name']."' value='".attr($res_pd['field_value'])."' ><span style='color:red;padding-left:25px;' >".text($query_oldpd['val'])."</span></td>
											<td><select name='".$res_pd['table_name']."-".$res_pd['field_name']."-sel'><option value='ignore' >".xlt('Ignore')."</option><option value='update' >".xlt('Update')."</option></select></td>";
									}
									if($i%2 == 0){
										if($i%4 == 2){
							...
							while($res_existing_prob = array_shift($result)){
								...
								foreach($aud_res['lists1'] as $k=>$v){
									...
									if(in_array($res_existing_prob['diagnosis'],$aud_res['lists1'][$k])){
										...
										<td>".xlt('Code')."</td><td><input type='text' name='lists1-diagnosis[]' value='".attr($aud_res['lists1'][$k]['diagnosis'])."' >
										...
										<tr style='color:red' ><td>&nbsp;</td><td>".text($res_existing_prob['title'])."</td><td>&nbsp;</td><td>".text($res_existing_prob['diagnosis'])."</td>
			...
			<td>
                ...
                <td>".xlt('Diagnosis')."</td><td>".text($res_existing_alerts['diagnosis'])."</td>
                  ...
                  <td>".xlt('Diagnosis')."</td><td><input type='text' name='lists2-diagnosis[]' value='".attr($val['diagnosis'])."' ></td>
									...
									<input type='hidden' name='lists2-type[]' value='".attr($val['type'])."' >
									</tr>";
							}
						?>
					</tr>
						...
						<?php
							...
							while($res_existing_medications = array_shift($result)){
								...
								echo "<tr><td>".xlt('Name')."</td><td>".text($res_existing_medications['drug'])."</td>
									...
									<td>".xlt('Status')."</td><td>".xlt($activity)."</td><td rowspan='2' >&nbsp;</td></tr><tr><td>".xlt('Form')."</td><td>
									".text($res_existing_medications['form'])."&nbsp;&nbsp;&nbsp;".xlt('Strength').
									"&nbsp;&nbsp;&nbsp;".text($res_existing_medications['size'])."</td>
							...
							foreach($aud_res['prescriptions'] as $key=>$val){
								...
								echo "<tr><td>".xlt('Name')."</td><td><input type='text' name='prescriptions-drug[]' value='".attr($val['drug'])."' ></td>
									...
									<input type='text' size='8' name='prescriptions-form[]' value='".attr($val['form'])."' >&nbsp;&nbsp;&nbsp;".xlt('Strength').
									"&nbsp;&nbsp;&nbsp;<input type='text' size='7' name='prescriptions-size[]' value='".attr($val['size'])."' ></td>
