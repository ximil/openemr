...
<script type="text/javascript">
  ...
  function find_all(){ // for each report the function mark_hilight() is called
    ...
    match = keys.match(/[\^\$\.\|\?\+\(\)\\~`\!@#%&\+={}<>]{1,}/);
    if(match){
      document.getElementById('alert_msg').innerHTML='Special characters are not allowed ..!';
      return;
    }
    else{
    ...
    if($j('.hilite').length <1){
      if(keys != '')
      document.getElementById('alert_msg').innerHTML='No results found..!';
    }
    else{
      document.getElementById('alert_msg').innerHTML='';
...
<div class="report_search_bar" style="width:100%;" id="search_options">
  <table style="width:100%;">
    <tr>
      <td>
        <input type="text" onkeyup="remove_mark_all();find_all();" name="search_element" id="search_element" style="width:200px;"/>
      </td>
      <td>
         <a class="css_button" onclick="remove_mark_all();find_all();" ><span>Find</span></a>
      </td>
      <td>
        <input type="checkbox" onclick="remove_mark_all();find_all();" name="search_case" id="search_case" />
      </td>
      <td>
        <span>Match case</span>
      </td>
      <td style="padding-left:10px;">
        <span class="text"><b>Search In:</b></span>
        <br>
        <?php
        $form_id_arr = array();
        $form_dir_arr = array();
        ...
        foreach ($ar as $key_search => $val_search) {
            ...
            if (($auth_notes_a || $auth_notes || $auth_coding_a || $auth_coding || $auth_med || $auth_relaxed)) {
                        ...
                        $form_id_arr[] = $res_search[2];
                         $form_dir_arr[] = $res_search[1];
                         //if($res_search != '' && $res_search != $res_search[1] ){
                         // $array_key_id[$res_search[1]][] = $res_search[2];
                         //}
                         //else{
                         // $array_key_id[$last_key][] = $res_search[2];
                         //}
                         //$last_key = $res_search[1];
            }
        }
        ...
        if(sizeof($form_id_arr)>0){
          ...
          echo "<select multiple size='4' style='width:300px;' id='forms_to_search' onchange='remove_mark_all();find_all();' >";
          while($res_forms_ids = sqlFetchArray($arr)){
            echo "<option value='".$res_forms_ids['formdir']."' selected>".$res_forms_ids['form_name']."</option>";
          }
          echo "</select>";
        }
        ?>
        ...
        if ($key == "documents") {
          ...
          if ($PDF_OUTPUT && $extension == ".pdf") {
            ...
            if (($auth_notes_a || $auth_notes || $auth_coding_a || $auth_coding || $auth_med || $auth_relaxed)) {
                ...
                ?>                
                <div name="search_div" id="search_div_<?php echo $form_id?>_<?php echo $res[1]?>" class="report_search_div class_<?php echo $res[1]; ?>">
                <?php
