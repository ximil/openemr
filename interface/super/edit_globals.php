...
foreach ($GLOBALS_METADATA as $grpname => $grparr) {
 if ( $_GET['mode'] != "user" || ($_GET['mode'] == "user" && in_array($grpname, $USER_SPECIFIC_TABS)) ) {
  ...
  echo " </table>\n";
  if(trim(strtolower($grpname)) == 'portal'){
    echo "<input style='position:absolute;top: 119px; left: 67%;' type='submit' name='form_download' value='".xlt('Download Offsite Portal Connection Files')."' />";
  }
  echo " </div>\n";
 }
}
...
<p>
 <input type='submit' name='form_save' value='<?php echo xla('Save','e'); ?>' />
</p>
</center>
