...
function ar_get_invoice_summary($patient_id, $encounter_id, $with_detail = false) {
  ...
  $resPCP = sqlStatement("
      SELECT post_time, co.code_type, co.code, co.modifier, co.code_text, pay_amount AS fee , units, account_code
  ...
  while ($rowPCP = sqlFetchArray($resPCP)) {
    ...
    if ($with_detail) {
      if (! $codes[$code]['dtl']) $codes[$code]['dtl'] = array();
      $tmp = array();
      if ($rowPCP['code_type'] == 'COPAY') {
        $tmp['pmt'] = 0 - $amount;
        $tmp['src'] = 'Pt Paid';
        $tmp['plv'] = 0;

	    // store table name and ID for this row (needed for edit & delete option in EOB invoice page)
	    // Note: We only save this info for CO-PAY rows in billing table.  The other rows in billing
        // table are chrages and cannot be deleted or modified from EOB invoice page 
	    // 7/6/2010   HB
        $tmp['tab'] = 'billing';
        $tmp['id'] = $rowPCP['id'];
