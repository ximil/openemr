...
function generate_select_list($tag_name, $list_id, $currvalue, $title, $empty_name = ' ', $class = '',
	...
	*/
	if (!$got_selected && strlen($currvalue) > 0)
	{
	  ...
	  echo "<option value='$optionValue' selected>" . htmlspecialchars( xl_list_label($lrow_inactive['title']), ENT_NOQUOTES) . "</option>\n";
	  $got_selected = TRUE;
	}

	if (!$got_selected && strlen ( $currvalue ) > 0 && !$multiple) {
		$list_id = $backup_list;
		$lrow = sqlQuery("SELECT title FROM list_options WHERE list_id = ? AND activity=1 AND option_id = ?", array($list_id,$currvalue));
