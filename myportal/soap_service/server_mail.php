...
public function getMails($data){
    if(UserService::valid($data[0])){
      ...
      if($data[2] == "inbox"){
        if($data[3] && $data[4]){
          $result_notes = getPatientNotes($data[1],'','0',$data[3]);
          $result_notifications = getPatientNotifications($data[1],'','0',$data[4]);
          $result = array_merge((array)$result_notes,(array)$result_notifications);
        }else{
          $result_notes = getPatientNotes($data[1]);
          $result_notifications = getPatientNotifications($data[1]);
          $result = array_merge((array)$result_notes,(array)$result_notifications);
        }
        return $result;
      }elseif($data[2] == "sent"){
        if($data[3]){
          $result_sent_notes = getPatientSentNotes($data[1],'','0',$data[3]);
        }else{
          $result_sent_notes = getPatientSentNotes($data[1]);
        }
  ...
  public function sendMail($data){
    if(UserService::valid($data[0])){
      ...
      foreach($to_list as $to){
        addMailboxPnote($data[1],$data[4],'1','1',$data[3],$to);
      }
      return 1;
    }else{
      throw new SoapFault("Server", "credentials failed in getMailDetails error message");
