...
//
require_once(dirname(__FILE__) . "/../../cron/config.php");
  ...
  public function print_ccr_report($data){
    ...
    public function ccdaFetching($data)
    {
        ...
        if (UserService::valid($data[0])=='existingpatient') {
            ...
            try {
                $ch = curl_init();
                $url =  $GLOBALS['web_root'] . "/interface/modules/zend_modules/public/encounterccdadispatch/index?cron=1&pid=$pid&site=$site_id";
    ...
    public function getEventLog($data)
    {
       global $pid;
        if (UserService::valid($data[0])=='existingpatient') {
            ...
            if(!empty($keyword)) {
                $sql .= " AND (log.date LIKE '%" . $keyword . "%' 
                            OR LOWER(event) LIKE '%" . strtolower($keyword) . "%'
                            OR LOWER(user) LIKE '%" . strtolower($keyword) . "%'  
                                OR LOWER(CONCAT(fname, ' ', lname)) LIKE '%" . strtolower($keyword) . "%'  
                            OR LOWER(groupname) LIKE '%" . strtolower($keyword) . "%'  
                            OR LOWER(comments) LIKE '%" . strtolower($keyword) . "%'
                            OR LOWER(user) LIKE '%" . strtolower($keyword) . "%' 
                            ) ";
            }
